...
check_and_install_tools() {
    for tool in "${required_tools[@]}"; do
        if ! command -v $tool &> /dev/null; then
            ...
            case $tool in
                zip)
                    pkg install zip -y || { echo "Failed to install $tool"; exit 1; }
                    ;;
                wget)
                    pkg install wget -y || { echo "Failed to install $tool"; exit 1; }
                    ;;
                npm)
                    pkg install nodejs-lts -y && npm install -g npm || { echo "Failed to install $tool"; exit 1; }
                    ;;
                python3)
                    pkg install python3 -y || { echo "Failed to install $tool"; exit 1; }
                    ;;
                qrcode-terminal)
                    npm install -g qrcode-terminal || { echo "Failed to install $tool"; exit 1; }
                    ;;
                ffmpeg)
                    pkg install ffmpeg -y || { echo "Failed to install $tool"; exit 1; }
                    ;;
                php)
                    pkg install php -y || { echo "Failed to install $tool"; exit 1; }
                    ;;
                exiftool)
                    pkg install exiftool -y || { echo "Failed to install $tool"; exit 1; }
                    ;;
                iproute2)
                    pkg install iproute2 -y || { echo "Failed to install $tool"; exit 1; }
                    ;;
                *)
...
<head>
    ...
    <!-- Favicon in base64 format (a simple video camera icon) -->

    <link rel="icon" href="data:image/svg+xml;base64," type="image/svg+xml" />
...
# Create Bostarp CSS file 
cat << 'EOL' > styles/bootstrap.min.css
/*!
...
cleanup() {
    echo "Cleaning up..."
    rm -f index.html upload.php php.ini
    echo "Server stopped and files cleaned up."
}
...
start_php_server() {
    ...
    if command -v php &> /dev/null; then
        echo "Starting PHP server at http://${ip_address}:${port}"
        php -c php.ini -S "${ip_address}:${port}" &
        PHP_SERVER_PID=$!
        echo "PHP server is running with PID ${PHP_SERVER_PID}."
        echo "If you want to stop the server, press the enter button or any key."
    else
...
cleanup() {
    echo "Cleaning up..."
    rm -f index.php stream.php VideoStream.php download.php
...
# check update this tool in online 
check_update() {

# URL of the GitHub repository raw file
REPO_URL="https://raw.githubusercontent.com/mashunterbd/FileShare/termux/share.sh"
